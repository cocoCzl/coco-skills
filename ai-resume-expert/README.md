...
完整初稿至少需要：目标岗位与职级、基本时间线，以及每段入选经历中已确认的个人角色、关键行动/技术方案和真实结果；核心技能需有经历支撑，并且没有未解决冲突或保密阻断。对有经验候选人，默认使用独立的“工作经历”和“重点项目经历”：前者用 2～4 条 STAR 成果说明公司、职位、业务方向、职责级别与持续价值，后者展开代表项目的背景、技术方案/取舍、个人贡献和结果；项目标题标明所属公司与任职时间，且两章不得重复。每个入选项目展示 5～10 项实际相关技术，页首核心技能不等于依赖清单。信息不足时只交付素材清单、缺口和骨架，不会把占位符集合称作初稿。

## 可能得到的产物

根据模式和阶段，输出以下适用子集。每次首次交付都会在正文外明确版本、内容状态、阻断项和下一步；“岗位基准版”不等于最终版或投递就绪：
