...
完整初稿至少需要：目标岗位与职级、基本时间线，以及每段入选经历中已确认的个人角色、关键行动/技术方案和真实结果；核心技能需有经历支撑，并且没有未解决冲突或保密阻断。资深工程师的项目放在对应公司经历下：工作概述写职责主线，项目写背景、精选技术栈、个人动作与结果。每个入选项目展示 5～10 项实际相关技术，页首核心技能不等于依赖清单。信息不足时只交付素材清单、缺口和骨架，不会把占位符集合称作初稿。
